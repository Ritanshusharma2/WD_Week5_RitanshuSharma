...
// Input Event: Tracks real-time typing in the text field
taskInput.addEventListener('input', (e) => {
    const currentLength = e.target.value.trim().length;
    // Optional: You can log or trigger UI feedback based on input length
    if (currentLength > 50) {
        taskInput.style.borderColor = '#ef4444'; // turn red if too long
    } else {
        taskInput.style.borderColor = '#cbd5e1';
    }
});

// Form Event: Handle submission
...
// Dynamic User Interaction & Event Delegation (Click Events)
taskList.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('delete-btn')) {
        tasks = tasks.filter(task => task.id !== id); // Removing elements
        saveToLocalStorage();
        renderTasks();
        return;
    }
...
// Button Click Events for filters
